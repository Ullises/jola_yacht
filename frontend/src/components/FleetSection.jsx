...
export const FleetSection = () => {
  ...
  const scrollContainer = (direction) => {
    if (direction === 'left') {
      ...
      setCurrentIndex(prev => Math.min(Math.max(0, fleet.length - 3), prev + 1));
    }
  };
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          ...
          {/* Cards Container */}
          <div className="overflow-hidden px-4 -mx-4 py-5 -my-5">
            <div
              className="flex transition-transform duration-500 ease-in-out"
              style={{ transform: `translateX(-${currentIndex * (100 / 3)}%)` }}
            >
            {fleet.map((item, index) => (
              <div key={item.id} className="flex-none w-1/3 px-4">
              <div
                data-testid={`fleet-card-${index}`}
                className="rounded-3xl overflow-hidden bg-white shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-100 group h-full"
              >
                {/* Image */}
